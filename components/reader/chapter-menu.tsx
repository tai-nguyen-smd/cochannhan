...
export function ChapterMenu({
  ...
  const readProgress = Math.round(
    (Number(currentChapter.order) / chapterList.length) * 100
  );

  const handleChapterSelect = (chapter: ChapterListItem) => {
    router.push(`/${bookSlug}/${chapter.slug}`);
    setIsOpen(false);
  };
        ...
        <SheetHeader className="p-4 border-b">
          ...
          <div className="space-y-2 mt-3">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>Chương {currentChapter.order}</span>
              <span>{chapterList.length} chương</span>
            </div>
          </div>
        </SheetHeader>
