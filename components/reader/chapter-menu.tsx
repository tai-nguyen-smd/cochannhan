...
export function ChapterMenu({ book, currentChapter }: ChapterMenuProps) {
  ...
  useEffect(() => {
    ...
    const savedBookmarks = localStorage.getItem(`book-${bookSlug}-bookmarks`)
    if (savedBookmarks) {
  ...
  const toggleBookmark = (chapterId: number) => {
    ...
    localStorage.setItem(`book-${bookSlug}-bookmarks`, JSON.stringify(newBookmarks))
  }
  ...
  const readProgress = Math.round((currentChapter.id / book.chapters.length) * 100)

  const handleChapterSelect = (chapter: Chapter) => {
    router.push(`/book/${bookSlug}/${chapter.slug}`)
    setIsOpen(false)
  }
        ...
        <SheetHeader className="p-4 border-b">
          ...
          <div className="space-y-2 mt-3">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>Chương {currentChapter.id}</span>
                ...
                <div key={chapter.id} className="flex items-center gap-2">
                  <Button
                    ...
                    className={cn(
                      ...
                      currentChapter.id === chapter.id && "bg-accent text-accent-foreground",
                    )}
                  >
